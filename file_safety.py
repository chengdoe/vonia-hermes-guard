...
_PROTECTED_FAMILY_SEPARATORS = (".", "-", "_", "~")


def _matches_protected_family(name: str, protected_name: str) -> bool:
    candidates = {name}
    if name.startswith("."):
        candidates.add(name[1:])
    return any(
        candidate == protected_name
        or any(candidate.startswith(protected_name + sep) for sep in _PROTECTED_FAMILY_SEPARATORS)
        for candidate in candidates
    )


def _is_secret_env_basename(name: str) -> bool:
    # Hermes documents .env.example as the safe, non-secret shape substitute.
    if name == ".env.example":
        return False
    return any(_matches_protected_family(name, base) for base in _SECRET_ENV_BASENAMES)


def _is_protected_hermes_write_path(resolved: str, base_real: str) -> bool:
    target = Path(resolved)
    base = Path(base_real)
    for relative in _PROTECTED_HERMES_WRITE_FILES:
        protected = Path(os.path.realpath(base / relative))
        if target == protected:
            return True
        if target.parent == protected.parent and _matches_protected_family(target.name, protected.name):
            return True
    for relative in _PROTECTED_HERMES_WRITE_DIRS:
        protected_dir = Path(os.path.realpath(base / relative))
        if target == protected_dir or protected_dir in target.parents:
            return True
    return False
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    if _is_secret_env_basename(Path(resolved).name):
        return "credential"
    ...
    for base_real in hermes_dirs:
        if _is_protected_hermes_write_path(resolved, base_real):
            return "credential"
